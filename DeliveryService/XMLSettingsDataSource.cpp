...
bool XMLSettingsDataSource::SaveCertificateSettings(const SoftingServerCertificateOwnSettings& certificatesSettings, const SoftingServerCertificatePkiLocationSettings& pkiLocation, std::ostream& stream)
{
	pugi::xml_document doc;
	pugi::xml_node rootNode = doc.append_child("CertificateSettings");
	...
	pugi::xml_node pkiStoreNode = rootNode.append_child("PkiStore");
	pkiStoreNode.append_attribute("PkiTrustedStore").set_value(pkiLocation.trustedFolderPath.c_str());
	pkiStoreNode.append_attribute("PkiRevocationStore").set_value(pkiLocation.revocationFolderPath.c_str());
	pkiStoreNode.append_attribute("PkiRejectedStore").set_value(pkiLocation.rejectedFolderPath.c_str());
	doc.save(stream);
	return true;
}

bool XMLSettingsDataSource::SaveConnectionAttributes(const ConnectionAttributes& connectionAttributes, std::ostream& stream)
{
	pugi::xml_document doc;
	pugi::xml_node rootNode = doc.append_child("ConnectionSettings");
...
bool XMLSettingsDataSource::LoadCertificateSettings(SoftingServerCertificateOwnSettings& certificatesSettings, SoftingServerCertificatePkiLocationSettings& pkiLocation, std::istream& stream)
{
	pugi::xml_document doc;
	...
	pugi::xml_node rootNode = doc.child("CertificateSettings");
	pugi::xml_node certificateNode = rootNode.child("CertificatesAccessConfiguration");
	std::string certificate = std::string(certificateNode.attribute("CertificatePath").as_string());
	std::string certificatePassword = std::string(certificateNode.attribute("CertificatePassword").as_string());
	std::string privateKey = std::string(certificateNode.attribute("PrivateKeyPath").as_string());
	certificatesSettings.certificatePath = certificate;
	certificatesSettings.password = certificatePassword;
	certificatesSettings.password = certificatePassword;
	pugi::xml_node pkiStoreNode = rootNode.child("PkiStore");
	std::string trusted = std::string(pkiStoreNode.attribute("PkiTrustedStore").as_string());
	std::string revoced = std::string(pkiStoreNode.attribute("PkiRevocationStore").as_string());
	std::string rejected = std::string(pkiStoreNode.attribute("PkiRejectedStore").as_string());
...
bool XMLSettingsDataSource::LoadConnectionAttributes(ConnectionAttributes& connectionAttributes, std::istream& stream)
{
	pugi::xml_document doc;
	pugi::xml_parse_result res = doc.load(stream);
	if (!res) {
	...
	pugi::xml_node rootNode = doc.child("ConnectionSettings");
